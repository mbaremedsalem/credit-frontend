...
import AjoutCreditStep2 from "./AjoutCreditParticulierStep2";
...
const AjoutCreditParticulier = ({ client, onCloseModal, typeFile }: props) => {
  ...
  const showModal = (client: CLientT) => {
    if (Number(montant) <= 0) {
      return enqueueSnackbar("Veuillez entrer un montant valide !", {
        variant: "error",
      });
    }
    if (Number(duree) <= 0) {
      return enqueueSnackbar("Veuillez entrer une durée valide !", {
        variant: "error",
      });
    }
    ...
    <div className="flex flex-col gap-2 ">
      <div className="flex items-center justify-center space-x-2">
        <h1 className="text-center font-bold text-2xl underline">
          Ajout Crédit {typeFile === "entreprise" ? "Entreprise" : "Client"}
        </h1>
      ...
      <div className="grid grid-cols-2 gap-3 max-lg:grid-cols-1">
        ...
        <label>
          <span className="text-base font-bold">Nature de Crédit</span>{" "}
          <span style={{ color: "red" }}>*</span>
          <Select
            options={[
      ...
      <div className="grid grid-cols-2 gap-3">
        <label>
          <span className="font-bold text-base">Montant</span>{" "}
          <span style={{ color: "red" }}>*</span>
          <Input
            type="number"
            value={montant}
        ...
        <label>
          <span className="font-bold text-base">Durée</span>{" "}
          <span style={{ color: "red" }}>*</span>
          <Input
            value={duree}
      ...
      <div className="flex ">
        {/* <span className="text-[16px] font-bold">Document à Télécharger</span> {" "}<span style={{ color: 'red' }}>*</span> */}
      ...
      {/* { Object.values(uploadedFiles).flat().length >= 9 && memoType && montant &&duree &&avis && TypeCredit ? ( */}
      {memoType && montant && duree && avis && TypeCredit && nature ? (
        <div className="grid grid-cols-2 gap-2 mt-2">
          <Button className="secondary-button !h-[42px]" onClick={onCloseModal}>
            Annuler
          </Button>
          <Button
            className="auth-button !h-[42px]"
        ...
        <Button
          className="secondary-button !h-[42px] mt-2"
      ...
      >
        <AjoutCreditStep2
          closeFirstModal={onCloseModal}
          closeSecondModal={handleCancel}
...
export default AjoutCreditParticulier;
