...
import { typUpdateCredit, useUpdateCredit } from '../../../Services/Demandes/useUpdateCredit';
...
const RemonterStep2= ({ligne, closeSecondModal, credit, oncloseFirstModal}:props) => {
  ...
  const showDocument = (file: string | File) => {
    const url = typeof file === 'string' ? `${BaseUrl}${file}` : URL.createObjectURL(file);
...
oncloseFirstModal!(),
  ...
  (doc) => doc.createur?.post === "Analyse de Risque" || doc.createur?.post === "Directeur Risque"
);
  ...
  return (
    <div className="w-full  mx-auto p-6 bg-white shadow-lg rounded-md space-y-6">
      ...
      <div className="flex items-center justify-center space-x-3">
        <h1 className="text-3xl font-bold text-gray-800">Détails des modifications</h1>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-gray-800">
          <div>
            <span className="font-medium">Client :</span> {ligne?.client?.client_code}
          </div>
          <div>
            <span className="font-medium">Nom :</span> {ligne?.client?.nom}
          </div>
          <div>
            <span className="font-medium">Prenom :</span>  {ligne?.client?.prenom}
          </div>
          <div>
            <span className="font-medium">Telephone :</span> {ligne?.client?.tel}
          </div>
          <div>
            <span className="font-medium">Date Naissance :</span> {ligne?.client?.date_naissance ? ligne?.client?.date_naissance.slice(0,10) : ""}
          </div>
          <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-gray-800">
        <div>
          <span className="font-medium">Montant :</span> {formatMontant(Number(credit?.montant))} MRU
        </div>
        <div>
          <span className="font-medium">Durée :</span> {credit?.duree} mois
        </div>
        <div>
          <span className="font-medium">Status :</span> {ligne?.status === "EN_COURS" ? "En Cours" : ligne?.status} 
        </div>
      </div>
...
{credit?.memo && (
  <div>
    ...
    <div><span className='font-medium'>Reference :</span> <span>{ligne?.reference}22</span></div>
     </div>

<div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-gray-800">
        <div>
          <span className="font-medium">Type Crédit :</span> {credit?.type_credit} 
        </div>
        <div>
          <span className="font-medium">Nature Crédit :</span> {credit?.nature} 
        </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center space-x-2 text-gray-700">
          ...
          <span className="text-lg font-semibold">Informations sur les Importations</span>
        </div>

        {/* - -------------------------------------  importations exist deja ------------------------------------------- */}

  <div><h1 className='text-center font-bold text-lg'> Anciennes importations  </h1></div>
 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-4 text-sm text-gray-900">
    {docsNormaux.map((fileObj, idx) => (
      <motion.div initial={{ opacity: 0, x:  idx % 2 !== 0 ? "100vw" : "-100vw" }}
        animate={{ opacity: 1, x:  0 }} 
        exit={{ opacity: 0, x:  idx % 2 !== 0 ? "100vw" : "-100vw" }}
  transition={{ duration: 0.4, delay:0.3*idx }} key={fileObj.id} className="bg-white rounded-xl shadow p-4 space-y-2 hover:shadow-md transition-all duration-200">
  ...
  <div><h1 className='text-center font-bold text-lg'>Nouvelles importations </h1></div>
  ...
  {new Date(fileObj.date_creation).toLocaleString("fr-FR", {
    ...
    second: "2-digit",
      ...
      <div className='grid  grid-cols-2 gap-3'>
        ...
        <Button className='auth-button !h-[44px]' onClick={afficherModal}>Remonter</Button>
      </div>
        <Modal
                         ...
                         <GiConfirmed size={32} className="text-green-500" />
                            ...
                            {fullName}, êtes-vous sûr de remonter cela à Nouvau ? Si oui, sachez qu'on ne pourra plus le modifier après la remontée. 
                               </p>
